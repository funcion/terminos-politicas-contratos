# **TÉRMINOS Y CONDICIONES GENERALES DE DESARROLLO DEL SISTEMA**

**DESARROLLADOR:** Bigsaeta — bigsaeta.com  
*Propietario/Accionista: Luis Figuera*  
**CLIENTE:** [Identificación del Cliente según facturación]

El presente documento constituye un **acuerdo legalmente vinculante** entre el DESARROLLADOR y el CLIENTE. Al aceptar este documento, al realizar cualquier pago o al hacer uso del SISTEMA, el CLIENTE acepta la totalidad de estos Términos y Condiciones.
...
* **Backend:** Lógica de negocio con framework moderno  
* **Base de Datos:** Motor relacional con estructuras optimizadas  
* **Panel Administrativo:** Backoffice para gestión de la operación  
* **Frontend:** Interfaz web responsiva para usuarios finales  
* **Infraestructura:** Entorno containerizado (Docker), hosting compartido, servidor dedicado o VPS, según los requerimientos y necesidades del proyecto

Cualquier otro componente o funcionalidad no listada expresamente en este documento será considerado trabajo adicional y se facturará según la tarifa vigente (precio fijo por módulo o por hora, según determine el DESARROLLADOR).
...
* **Errores (bugs)** no detectados durante las pruebas  
* **Comportamientos inesperados** bajo condiciones no previstas  
* **Períodos de inactividad** por mantenimiento o fallos técnicos  
* **Problemas de rendimiento** bajo cargas de tráfico extraordinarias
...
* **Requiere mantenimiento constante** para seguir funcionando correctamente  
* **Depende de servicios externos** (servidores, bases de datos, APIs) que pueden cambiar sin previo aviso  
...
* Caídas del servidor o proveedor de servidor   
* Ataques informáticos (DDoS, hackeos, ransomware, etc.)  
* Cambios en servicios de proveedores externos  
* Cortes de energía o internet  
* Uso indebido o negligente del SISTEMA  
* Modificaciones al código realizadas por terceros no autorizados  
* Sobrecarga del sistema por volumen superior al planificado
...
Sin mantenimiento activo, el SISTEMA:

1. **Acumulará vulnerabilidades** de seguridad explotables  
2. **Perderá compatibilidad** con navegadores y dispositivos modernos  
3. **Sufrirá degradación** del rendimiento con el crecimiento de datos  
4. **Dejará de funcionar eventualmente** cuando sus dependencias queden obsoletas  
5. **No se adaptará** a cambios en servicios externos

El mantenimiento (preventivo, correctivo y evolutivo) se ofrece como servicio **separado** con tarifas y condiciones propias.
...
* El VPS **no cuenta con panel de administración** (cPanel o similar) y es gestionado **exclusivamente** por el DESARROLLADOR.  
* El CLIENTE **no tendrá acceso SSH, FTP, de administración de base de datos ni de ningún otro tipo** al servidor durante la fase de desarrollo. Este acceso solo se concedería, si el CLIENTE lo solicita y acuerda el costo del servicio, una vez verificado el pago total del proyecto y como parte de un proceso de migración o entrega formal. Todo despliegue, configuración y mantenimiento del servidor durante el desarrollo es competencia exclusiva del DESARROLLADOR.  
* Los costos de servidor serán facturados al CLIENTE de forma separada.  
* Es responsabilidad del CLIENTE **renovar los pagos de servicios** (servidor, dominio, certificados SSL, etc.) **antes de su vencimiento**. Si el CLIENTE no renueva a tiempo, el DESARROLLADOR se reserva el derecho de **suspender el servicio** sin responsabilidad por la interrupción.  
...
* Una vez migrado, el DESARROLLADOR **no se responsabiliza** del rendimiento, disponibilidad o seguridad del servidor del CLIENTE.  
* El servidor del CLIENTE deberá cumplir con los requisitos técnicos mínimos que el DESARROLLADOR indicará.
...
* El SISTEMA está construido sobre una **base de código preexistente** propiedad del DESARROLLADOR.  
* El DESARROLLADOR **opera sus propios proyectos y plataformas** de forma independiente y tiene el derecho irrevocable de continuar desarrollándolos, modificándolos y comercializándolos libremente, incluso si compiten directa o indirectamente con el negocio del CLIENTE.  
* El CLIENTE **no puede restringir, prohibir ni reclamar exclusividad** sobre el desarrollo de proyectos similares por parte del DESARROLLADOR.  
...
### **7.5. Recursos de Terceros y Licencias Comerciales**
...
* **No modificar el código fuente** sin autorización o sin un desarrollador calificado (anula la garantía automáticamente).  
* Reportar incidencias de forma **organizada** con evidencia (capturas, pasos para reproducir).  
* Obtener las **licencias y permisos legales** necesarios para operar su negocio o actividad comercial en su jurisdicción, cualquiera sea su naturaleza.  
* Cumplir con las **leyes de protección de datos** aplicables.  
* Mantener las **credenciales de acceso seguras**.  
* Responder solicitudes del DESARROLLADOR en un plazo máximo de **48 horas hábiles**.  
* Mantener el **saldo positivo** en las plataformas de terceros (SMS, WhatsApp, servidor). El DESARROLLADOR no es responsable por interrupciones derivadas de falta de pago del CLIENTE a dichos proveedores.  
...
El CLIENTE queda expresamente autorizado a presentar el código fuente y la documentación técnica del SISTEMA ante los organismos reguladores que correspondan según la naturaleza de su actividad comercial, con el único propósito de cumplir con los requisitos de registro y legalización de su operación.

El CLIENTE será el único responsable de:

1. Gestionar dicho registro por su cuenta y costo.  
2. Asegurar que los acuerdos de confidencialidad con el ente regulador protejan adecuadamente el código.  
3. Asumir cualquier consecuencia derivada de la exposición del código, incluyendo pero no limitado a filtraciones, copias no autorizadas o mal uso de la información por parte de terceros, eximiendo al DESARROLLADOR de toda responsabilidad al respecto.

El DESARROLLADOR, a solicitud del CLIENTE y por un costo adicional facturable por horas, podrá brindar asesoría técnica para preparar la documentación requerida, sin que ello implique asumir responsabilidad alguna ante el ente regulador.
...
* Pérdidas financieras del negocio del CLIENTE  
* Pérdida de datos por falta de backups o fallos del servidor  
* Daños reputacionales del negocio  
* Reclamaciones de terceros (usuarios de la plataforma) contra el CLIENTE  
* Fraudes, pagos duplicados o disputas de pago (la verificación manual es responsabilidad del CLIENTE)  
* Incumplimiento regulatorio del negocio

El SISTEMA es una **herramienta de registro y gestión**; la validación final de la disponibilidad de fondos en las cuentas bancarias o billeteras electrónicas es responsabilidad humana y exclusiva del CLIENTE antes de liberar créditos en la plataforma.

**Tope de responsabilidad:** La responsabilidad máxima del DESARROLLADOR no excederá el monto total pagado por el CLIENTE durante los seis (6) meses anteriores a la fecha en que se originó la reclamación.

**Prescripción / Caducidad:** Salvo lo dispuesto en el párrafo siguiente, el CLIENTE renuncia expresamente a los lapsos de prescripción ordinarios establecidos en la ley y acepta que cualquier acción legal o reclamo contra el DESARROLLADOR derivado de este contrato o del SISTEMA deberá ser iniciada dentro del plazo máximo de **un (1) año** contado desde la fecha en que ocurrieron los hechos que dan origen a la reclamación. Transcurrido dicho plazo sin que se haya interpuesto la acción, el derecho del CLIENTE para reclamar caducará irremediablemente, operando la extinción automática de la obligación reclamada. 

**Excepción por vicios ocultos:** Para reclamaciones basadas en **vicios o defectos ocultos** del SISTEMA que no hayan sido ni pudieran haber sido detectados mediante pruebas razonables durante el período de garantía o aceptación, el plazo de prescripción será de **dos (2) años** contados desde la entrega del módulo o funcionalidad donde se detecte el vicio. Esta excepción no aplica a errores estéticos, cambios de lógica, funcionalidades no acordadas, ni a reclamaciones de cualquier naturaleza una vez transcurridos 2 años desde la finalización del proyecto.
...
* El DESARROLLADOR puede cobrar una **tarifa de diagnóstico** si debe revisar código modificado por terceros.
...
La gestión de la **reputación**, credenciales, cuentas y saldos ante proveedores externos es **responsabilidad exclusiva del CLIENTE**. Si un proveedor bloquea, suspende o limita el servicio por uso indebido, incumplimiento de sus políticas, falta de pago o cualquier otra causa atribuible al CLIENTE, el DESARROLLADOR no asume responsabilidad alguna por la interrupción del servicio.
...
El CLIENTE reconoce expresamente que la actividad comercial para la cual utilizará el SISTEMA puede estar sujeta a regulaciones específicas, prohibiciones o requisitos de licencia en diversas jurisdicciones, y que es su exclusiva responsabilidad determinar la legalidad de su operación y obtener las autorizaciones correspondientes, sin que el DESARROLLADOR tenga obligación ni responsabilidad alguna al respecto.
...
* El incumplimiento por parte del CLIENTE de cualquier ley, regulación o normativa aplicable a su negocio.
* Cualquier reclamo de usuarios finales, autoridades gubernamentales, entidades regulatorias o terceros relacionados con la operación del SISTEMA.
* Cualquier disputa fiscal, tributaria o cambiaria derivada de las transacciones realizadas a través del SISTEMA.
* Cualquier investigación administrativa, penal o civil relacionada con la operación del negocio del CLIENTE.
* Cualquier violación de derechos de propiedad intelectual, derechos de autor, marcas o imágenes de terceros por parte de los materiales suministrados por el CLIENTE.
...
El CLIENTE reconoce que el SISTEMA procesa datos sensibles de terceros (usuarios finales). El CLIENTE asume la figura legal de **Responsable del Tratamiento de Datos**, con todas las obligaciones que ello conlleva bajo la Ley de Protección de Datos Personales y demás normativa aplicable. El DESARROLLADOR actúa únicamente como proveedor tecnológico y no tiene control, acceso ni responsabilidad sobre las políticas de privacidad, los consentimientos de usuarios, o el uso que el CLIENTE haga de la información recolectada. El DESARROLLADOR implementará medidas de seguridad estándar de la industria, pero **no garantiza la invulnerabilidad absoluta** frente a ataques sofisticados, inyección de código por descuido del administrador o fallos en el servidor. El DESARROLLADOR no será responsable por la pérdida, robo o filtración de bases de datos, ni por fraudes económicos realizados por usuarios dentro de la plataforma.

**Propiedad de los datos:** Tras la resolución del contrato por mutuo acuerdo o finalización exitosa, y previo pago total de todas las obligaciones, el CLIENTE es el propietario de la base de datos de los usuarios de su plataforma. En caso de rescisión por incumplimiento del CLIENTE (mora o falta de pago), la base de datos no será entregada, formando parte de los activos retenidos según la cláusula 17.2. Sin embargo, el DESARROLLADOR **no tiene obligación de almacenar, mantener ni entregar** dicha información transcurridos **30 días** desde la finalización de la relación contractual.

**Auditoría de seguridad:** El DESARROLLADOR garantiza la corrección de fallos de lógica de programación, mas **no la invulnerabilidad** ante ataques dirigidos o vulnerabilidades de día cero (zero-day). Es responsabilidad del CLIENTE contratar, si lo desea, una **auditoría de seguridad externa (pentesting)** por cuenta propia. El DESARROLLADOR colaborará facilitando el acceso necesario para dicha auditoría.

---

## **15\. PROHIBICIÓN DE INGENIERÍA INVERSA Y REVENTA**

El SISTEMA contiene propiedad intelectual y metodologías propias del DESARROLLADOR. El CLIENTE tiene prohibido:

1. Realizar **ingeniería inversa** para replicar el código base.  
2. **Comercializar, distribuir ni sublicenciar** el código fuente del SISTEMA como un producto SaaS o licencia de software a terceros no afiliados.  
3. Crear **productos derivados para su venta** basados en el código entregado sin un acuerdo de comercialización aparte.

El uso del código está limitado a la **operación del propio negocio del CLIENTE**. El incumplimiento de esta cláusula faculta al DESARROLLADOR a revocar la licencia de uso y emprender acciones legales.

---

## **16\. SUBCONTRATACIÓN Y COLABORADORES**

El DESARROLLADOR podrá proponer la incorporación de colaboradores o subcontratistas para tareas específicas. Bajo esta modalidad:

* **Naturaleza de la Intervención:** El DESARROLLADOR actúa exclusivamente como intermediario y coordinador técnico. La decisión final de contratar y aprobar el trabajo de dichos colaboradores es responsabilidad del CLIENTE.  
* **Gestión de Pagos:** El CLIENTE cubrirá los honorarios de los colaboradores a través del DESARROLLADOR. Este último actúa como mero gestor de pagos para facilitar la administración del proyecto.  
* **Supervisión Técnica y Responsabilidad:** El DESARROLLADOR coordina a los colaboradores. Si bien el CLIENTE aprueba los costos, el DESARROLLADOR mantiene la supervisión técnica final. Cualquier error será corregido bajo los términos de garantía habituales establecidos en este contrato, salvo que el error provenga de instrucciones directas del CLIENTE al colaborador sin consulta previa al DESARROLLADOR. El CLIENTE asume el riesgo derivado de la contratación si decide omitir la supervisión del DESARROLLADOR.  
* **Relación Contractual:** No existe relación laboral ni contractual directa entre el colaborador y el CLIENTE. El colaborador es un recurso independiente coordinado por el DESARROLLADOR.  
* **Confidencialidad:** El DESARROLLADOR se asegura de que los colaboradores firmen acuerdos de confidencialidad equivalentes a los de este contrato.  
* **No Circunvención:** El CLIENTE no podrá contactar, contratar ni encargar trabajo directa o indirectamente a los colaboradores propuestos por el DESARROLLADOR sin autorización previa y por escrito de este. El incumplimiento de esta cláusula se considerará falta grave y faculta al DESARROLLADOR a facturar al CLIENTE, como penalidad y sin necesidad de constitución en mora, el equivalente a **tres (3) meses de honorarios** del colaborador circunvenido, calculados sobre la base de la tarifa promedio que dicho colaborador estuvo facturando al proyecto. Esta penalidad es independiente de cualquier otra acción legal que el DESARROLLADOR pueda emprender.

---

### **17.1. Rescisión por Voluntad del CLIENTE o Mutuo Acuerdo:**

Si el CLIENTE decide terminar el proyecto de forma anticipada, o si ambas partes acuerdan la rescisión, el CLIENTE deberá liquidar el saldo total de las horas trabajadas hasta el último día de servicio. Una vez verificado este pago final, el DESARROLLADOR entregará el código fuente en el estado en que se encuentre, sin obligación de completar funcionalidades pendientes, mediante un archivo comprimido (.zip) que incluirá todos los archivos del código fuente y la base de datos en formato .sql.
...
* **Confidencialidad:** El CLIENTE se obliga a mantener en estricta reserva y confidencialidad toda la información técnica, comercial, algorítmica, arquitectónica, metodológica y de cualquier naturaleza relacionada con el SISTEMA, el código fuente (incluyendo su estructura, lógica y patrones), las herramientas, librerías personalizadas, configuraciones, credenciales de acceso y cualquier otro elemento del proceso de desarrollo al que tenga acceso, por un período de **5 años** contados desde la finalización del proyecto. Para secretos comerciales, algoritmos propietarios, lógica de negocio del código base del DESARROLLADOR y metodologías de desarrollo, la obligación de confidencialidad es **indefinida** y subsiste incluso después de la terminación del contrato por cualquier causa. El DESARROLLADOR mantendrá en reserva la información comercial del CLIENTE que expresamente le sea marcada como confidencial, por el mismo período de 5 años.
* **Fuerza Mayor:** Ninguna parte es responsable por incumplimiento causado por desastres naturales, pandemias, cortes masivos u otros eventos imprevisibles. Dada la ubicación de las partes, se consideran expresamente eventos de fuerza mayor: **racionamientos eléctricos prolongados**, fallas masivas de conectividad de los ISP nacionales y cambios súbitos en las regulaciones de divisas que impidan el cumplimiento de los plazos.  
* **Resolución de Conflictos:** Se intentará negociación directa por 30 días. De no resolverse, el DESARROLLADOR podrá optar, a su exclusiva discreción, por someter la controversia a: (a) los tribunales ordinarios de **Barcelona, Venezuela**, con renuncia expresa del CLIENTE a cualquier otro fuero o jurisdicción; o (b) un arbitraje de equidad ante un único árbitro designado de común acuerdo, sin necesidad de formalidades procesales.  
...
*Bigsaeta — Términos y Condiciones Generales de Desarrollo de Software.*
